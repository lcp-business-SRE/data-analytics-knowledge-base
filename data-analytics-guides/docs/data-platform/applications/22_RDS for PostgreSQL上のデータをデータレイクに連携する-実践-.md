...
この記事ではRDS for postgreSQLからデータ基盤に向けてデータを連携する方法の実装に関して説明する。
前回の記事[^1]ではRDS起点のデータ連携方式を検討し、各方式のメリット・デメリットを比較した。
最終的に以下のような**RDSのスナップショットのGlue経由のS3 Tables連携**を検証構成として採用することにした。
**概念図**
![概念図](../assets/poc-structure-detail.drawio.png)
<details><summary>詳細なシステム構成図</summary>
...
AWSのインフラ管理を一通り知っている人が、今回検証したS3Tables[^2]もしくはIceberg[^3]テーブルにRDSのデータを連携するための手順を理解し、再現できるようにすること。

## 技術検証の内容

1. S3バケットと、S3 Tablesの準備
2. 連携に必要なIAM権限の用意
3. KMSキー準備
4. スナップショット取得
5. スナップショットのS3エクスポート
...
## S3バケットと、S3 Tablesの準備

S3バケットはRDSのスナップショットエクスポート先として使用する。
S3 Tablesはデータレイクとして使用する。
両方とも権限設定の際にリソースとして指定するため、このタイミングで準備しておく。
...
> 📝**補足**
> **Intelligent-Tiering Archive 設定**について
> S3バケットのライフサイクルポリシーで、Intelligent-Tiering Archive 設定を有効にすることができる。
> スナップショットのオリジナルを長期間保存することを考えている場合は有効化を検討したほうが良い

### S3 Tables準備

S3 Tablesに使用するS3のテーブルバケットを作成する。

1. S3 Tablesのコンソールに入り、左側のメニューからテーブルバケットを選択
2. 「テーブルバケットの作成」を選択
3. 必要な情報を入力し、テーブルバケットを作成する
設定内容
   - テーブルバケット名: 任意の名前
   - デフォルトストレージクラス: 標準/インテリジェント階層化
   - デフォルトの暗号化: 暗号化タイプを指定しない

> 📝**補足**
> **デフォルトストレージクラスの設定**について
>
> - 標準: 頻繁にアクセスされるデータ向け
> - インテリジェント階層化: アクセスパターンに基づいて自動的に最適なストレージクラスにデータを移動する
>
> PoCの場合は短期間で削除するため標準でも問題ないが、実運用でもあとから変更が可能なので最初は標準にして、運用を見ながらインテリジェント階層化に変更するのもよい

## 連携に必要なIAM権限の用意

Snapshotのエクスポート、Glueジョブの実行に必要なIAMロールを作成する。

> ℹ️**info**
>連携後のデータを閲覧、操作するにはLake Formationでの権限付与も必要になる。連携後のみ設定できる部分もあるため、[後ほど](#lake-formationでの権限付与)で説明する。
...
S3へのエクスポート時の暗号化に使用するKMSキーを作成する。

> 📝**メモ**
> 既に適切なKMSキーがある場合はこのセクションで作成する代わりに、そのキーの閲覧権限を先ほど作成したロールに付与してもよい
...
RDS上のデータを取得するためのスナップショットの取得を行う。
今回は手動取得したスナップショットを用いて解説する。

> 📝**補足**
> 自動取得されたスナップショットを使用する場合は別途スナップショットの識別子を確認する必要がある。
...
RDSのスナップショットはそのままでは他サービスが使用できないため、S3にエクスポートすることで活用できるようにする。
...
> **補足**
> Worker typeの指定について
> Worker typeの指定でインスタンスサイズが変更される。
> Glueのコストは「稼働時間」×「インスタンスサイズで決まる時間単価」で決まる。基本的にサイズを上げると一定規模までは処理時間も比例して短くなるため、適切な範囲で大きいインスタンスサイズを選択するほうが良い。

#### 4.スクリプト作成

AWS Glue（サーバレスETL）上で動作する PySpark ジョブを作成する。
今回はコンソール上で直接スクリプトを編集したが、CloudFormationやCDKなどで管理することも可能。
...
<details><summary>作成したデータ連携用スクリプトと設定 (クリックで展開)</summary>
...
S3 Tablesを用いると権限管理がIAMからLake Formation[^7]に委任される。そのため連携したデータの活用にはLake Formationでの閲覧権限付与をする必要がある。
...
- ロール
  - Athenaを使用する際に付与されるロール
    - AWS管理コンソールからAthenaを使用する場合は、操作ユーザーのロール
    - ほかのサービスからAthena経由で閲覧する場合は、そのサービスに付与されているロール
  - Glue ジョブに付与されているロール
    - テーブル自体を作成したロールは自動的に権限が付与されるはずだが、別のロールで作ったテーブルにアクセスする場合は付与する必要がある

**付与する際に指定するリソース**:

各カタログごとに以下のリソースに対して権限を付与する。
...
AthenaでS3 Tablesに連携したデータをSQLライクなクエリを用いて確認することが可能。
